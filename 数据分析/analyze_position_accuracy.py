...
def create_position_accuracy_table(cursor, table_name, db_type):
    """
    创建位置准确率表
    ...
    if db_type == 'bytes':
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {position_table_name} (
                bytes_key_position INTEGER PRIMARY KEY,
                bytes_frequency INTEGER NOT NULL,
                bytes_probability REAL NOT NULL,
                bytes_total_records INTEGER NOT NULL,
        ...
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {position_table_name} (
                tokens_key_position INTEGER PRIMARY KEY,
                tokens_frequency INTEGER NOT NULL,
                tokens_probability REAL NOT NULL,
                tokens_total_records INTEGER NOT NULL,
...
def insert_position_stats(cursor, position_table_name, position_frequency, total_records, db_type):
    """
    插入或更新位置准确率统计
    ...
    for key_position, frequency in position_frequency.items():
        ...
        if db_type == 'bytes':
            cursor.execute(f"""
                ...
                (bytes_key_position, bytes_frequency, bytes_probability, bytes_total_records, bytes_last_updated)
                VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
            """, (key_position, frequency, probability, total_records))
        else:  # tokens
            cursor.execute(f"""
                INSERT OR REPLACE INTO {position_table_name}
                (tokens_key_position, tokens_frequency, tokens_probability, tokens_total_records, tokens_last_updated)
                VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
            """, (key_position, frequency, probability, total_records))
...
def analyze_model_position_accuracy(model_db_path):
    ...
    for table_name, identifier in tables:
        ...
        # 分析位置准确率
        position_frequency, total_records = analyze_table(
            model_db_path, table_name, identifier, db_type
        )
        ...
        # 创建位置准确率表（写入独立结果数据库）
        position_table_name = create_position_accuracy_table(out_cursor, table_name, db_type)
        
        # 插入统计数据（写入独立结果数据库）
        insert_position_stats(out_cursor, position_table_name, position_frequency, total_records, db_type)
    ...
    print(f"每个数据表对应一个 _position_accuracy 表（保存在独立结果数据库中）")
    print("=" * 70)
...
def list_position_accuracy(model_or_result_db_path, table_name=None):
    ...
    for pos_table_name in position_tables:
        ...
        if 'bytes_key_position' in columns:
            cursor.execute(f"""
                SELECT bytes_key_position, bytes_frequency, bytes_probability, bytes_total_records
                FROM {pos_table_name}
                ORDER BY bytes_key_position
            """)
        else:  # tokens
            cursor.execute(f"""
                SELECT tokens_key_position, tokens_frequency, tokens_probability, tokens_total_records
                FROM {pos_table_name}
